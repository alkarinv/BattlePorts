...
import mc.org.khelekore.prtree.DistanceResult;
import mc.org.khelekore.prtree.NodeFilter;
...
class NearestNeighbour<T>
{
	...
	public NearestNeighbour(MBRConverter<T> converter, NodeFilter<T> filter,
			int maxHits, Node<T> root, DistanceCalculator<T> dc, PointND p)
	{
	...
	public List<DistanceResult<T>> find()
	{
		...
		MinDistComparator<T, Node<T>> nc = new MinDistComparator<T, Node<T>>(
				converter, p);
