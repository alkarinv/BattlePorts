...
public class PRTree<T>
{
	...
	public void load(Collection<? extends T> data)
	{
		...
		{
			height++;
			List<InternalNode<T>> internalNodes = new ArrayList<InternalNode<T>>(
					estimateSize(nodes.size()));
	...
	public MBR2D getMBR2D()
	{
		...
		if (mbr == null)
			return null;
		return new SimpleMBR2D(mbr.getMin(0), mbr.getMin(1), mbr.getMax(0),
				mbr.getMax(1));
	}
	...
	public List<DistanceResult<T>> nearestNeighbour(DistanceCalculator<T> dc,
			NodeFilter<T> filter, int maxHits, PointND p)
	{
		if (isEmpty())
			return Collections.emptyList();
		NearestNeighbour<T> nn = new NearestNeighbour<T>(converter, filter,
				maxHits, root, dc, p);
		return nn.find();
	}
}
