...
public class PortExecutor extends CustomCommandExecutor  {
	...
	public boolean portRename(CommandSender sender, Port port, String newName) {
		...
		return sendMessage(sender,"&ePort &6"+oldName+ "&e renamed &6" + newName);
	}
	...
	public boolean portSetOptions(CommandSender sender, Port port, String[] args) {
		...
		if (port.hasOption(PortOption.FIRSTUSE)){
			if (!(sender instanceof Player)){
				return sendMessage(sender, "&cYou need to be ingame to use this option");
			}
			Player p = (Player) sender;
			try {
				port.setDestination2(p.getLocation());
			} catch (Exception e) {
				return sendMessage(sender, "&4Cant set destination to be within the source");
			}
		}
	...
	public boolean portAlterFirstUse(Player p, Port port) {
		...
		return sendMessage(p, "&ePort first use destination set to this location");
	}
	...
	public boolean portInfo(CommandSender sender, Port port) {
		return sendMessage(sender,"&e - &2"+port.getName()+ "&e[&6"+port.getWorld().getName()+" "+ port.getSourceStr()+"&e] -> [" + port.getDestStr()+"&e]" + port.getOptionString());
	}
	...
	public boolean portList(CommandSender sender, String[] args) {
		...
		if (world != null){
			...
			sendMessage(sender,"&6/port list <worldname or all> &e: to list a world or all worlds");
		...
		for (World w: wmap.keySet()){
			...
			for (Port port: theports){
				sendMessage(sender,"&2"+port.getName()+" &e[&6"+ port.getSourceStr()+"&e] -> [" + port.getDestStr()+"&e]" + port.getOptionString());
			}
		}
		return true;
	}
	...
	public boolean portDelete(CommandSender sender, Port port) {
		pc.deletePort(port);
		return sendMessage(sender, "&eYou have deleted port &6" +port.getName());
	}

	@MCCommand(cmds={"source","alterSource"},op=true, usage=PORT_ALTER_SOURCE)
	...
	@MCCommand(cmds={"create"},inGame=true, op=true, usage=PORT_CREATE)
	public boolean portCreate(Player p, String name, String[] args) {
		...
		for (int i=2;i<args.length;i++){
			String op = args[i];
			if (!Port.checkOption(op)){
				sendMessage(p,"Port Option " + args[i]+ " is not a valid option");
				sendValidOptions(p);
				return true;
			}
			portOptions.add(op);
		...
		pc.addPort(port);
		sendMessage(p, ChatColor.AQUA + "The port "+port.getName() +" was created successfully. options="+port.getOptions());
		return true;
	}
	...
	private void sendValidOptions(CommandSender sender) {
		...
		for (PortOption po : PortOption.values()){
			String name = po.needsValue() ? po.getName()+"=<value>" : po.getName();
