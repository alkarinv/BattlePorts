...
//		sendMessage(sender,PORT_ADD_OPTIONS);
	...
	@MCCommand(cmds={"rename"}, inGame=true, alphanum={2}, usage=PORT_RENAME)
	...
	@MCCommand(cmds={"dest","alterDest"},inGame=true, usage=PORT_ALTER_DEST)
	public boolean portAlterDest(Player p, PlayerPort port) {
		if (!wgp.canBuild(p, p.getLocation())){
			return sendMessage(p, "&cThe portal destination must be somewhere you have build rights to");}
		try {
	...
	@MCCommand(cmds={"source","alterSource"}, inGame=true, usage=PORT_ALTER_SOURCE)
	public boolean portAlterSource(Player p, PlayerPort port) {
		...
		if (sel.getArea() > area) {
			return sendMessage(p, "&cYou can only make a portal of size &6"+area +"&c you selected &6" + sel.getArea());}

		if (!wgp.canBuild(p, sel.getMinimumPoint()) || !wgp.canBuild(p, sel.getMaximumPoint())){
			return sendMessage(p, "&cYou can't create a portal in a region you cant build in");
		}
		BoundingBox bb = new BoundingBox(sel.getMinimumPoint(), sel.getMaximumPoint());
	...
	public boolean portInfo(Player p, PlayerPort port) {
		sendMessage(p,"&e - &2"+port.getPortName()+ "&e[&6"+port.getWorld().getName()+" "+ port.getSourceStr()+"&e] -> [" + port.getDestStr()+"&e]" + port.getOptionString());
		return true;
	}
	...
	public boolean portList(Player p) {
		...
		for (World w: wmap.keySet()){
			...
			for (PlayerPort port: theports){
				sendMessage(p,"&2"+port.getPortName()+" &e[&6"+ port.getSourceStr()+"&e] -> [" + port.getDestStr()+"&e]" + port.getOptionString());
			}
		}
		sendMessage(p,"&eYou are using &6" + ports.size()+"/" + maxPorts+"&e ports");
		return true;
	}
	...
	@MCCommand(cmds={"create"}, inGame=true, usage=PORT_CREATE, perm="portal.create")
	public boolean portCreate(Player p, String name) {
		Port port = pc.getPlayerPort(p,name);
		if (port != null) {
			return sendMessage(p, "&4There is already a port with that name. Please pick a unique name.");
		}
		...
		if (pi.nports >= maxPorts){
			return sendMessage(p, "&cYou already have &6"+pi.nports+"/"+maxPorts+"&c ports");}
...
//			return sendMessage(sender, ChatColor.RED + "Only cuboid regions are supported.");
//		}
		int area = PermissionController.getAvailableArea(p);
		if (sel.getArea() > area) {
			return sendMessage(p, "&cYou can only make a portal of size &6"+area +"&c you selected &6" + sel.getArea());}
		...
		try {
			port.setSourceBox(p.getWorld(), sel.getMinimumPoint(), sel.getMaximumPoint());
		} catch (Exception e) {
			return sendMessage(p,"&4Source must have corners in the same world");
		}
		try {
			port.setDestination(p.getLocation());
		} catch (Exception e) {
		...
		pc.addPort(port);
		return sendMessage(p, "&2You created port &6"+name+"&2 successfully.");
	}
