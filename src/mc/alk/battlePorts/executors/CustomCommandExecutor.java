...
//		System.out.println(command.getName() +"   " + label + "      cmdlabel = " + command.getLabel());
		...
		for (Class<?> theclass : mwrapper.method.getParameterTypes()){
			try{
				if (CommandSender.class == theclass){
					...
					objs[objIndex] = verifyPlayerPort((Player)sender,args[strIndex++]);
				} else if (String[].class == theclass){
					objs[objIndex] = args;
				} else if (Object[].class == theclass){
					objs[objIndex] = args;
	...
	private Port verifyPort(String string) throws InvalidArgumentException {
		...
		if (p == null)
			throw new InvalidArgumentException("Port " + p+" can not be found");
		return p;
	}

	private PlayerPort verifyPlayerPort(Player player, String string) throws InvalidArgumentException {
