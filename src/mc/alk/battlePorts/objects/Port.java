...
public class Port {
	...
	public boolean withinSource(Location l){return source.contains(l);}

	public void setName(String name) {this.name = name;}
	public String getName() {return name;}

	public Location getSrcLowerCorner() {return source.getCorner1();}
	public Location getSrcUpperCorner() {return source.getCorner2();}

	public void setSourceBox(World world, Location l1, Location l2) throws Exception {
		Location c1 = new Location(world, l1.getBlockX(),l1.getBlockY(),l1.getBlockZ());
		Location c2 = new Location(world, l2.getBlockX(),l2.getBlockY(),l2.getBlockZ());
	...
	public String getDestStr() {
		return "&6"+(source.getWorld()==destination.getWorld() ? "" : "&5"+destination.getWorld().getName()+":") + "&6"+getLocString(destination);
	}
	public String getSourceStr() {
		return "&6"+getLocString(source.getCorner1()) + "&e<->&6"+getLocString(source.getCorner2());
	}
	public String getLocString(Location l){
		return l.getBlockX()+"," + l.getBlockY()+","+l.getBlockZ();
	}
	public String getOptionString() {
		return options == null || options.isEmpty() ? "" : "&5"+toString(options);
	}
	...
	private String toString(HashMap<PortOption, String> options2) {
		...
		for (PortOption po : options2.keySet()){
			if (!first) sb.append(", ");
			first = false;
			sb.append(po.getName() + (po.needsValue ? "="+options2.get(po) : ""));
		}
		sb.append("]");
		return sb.toString();
	}
		...
		for (PortOption po : options.keySet()){
			...
			case HASGROUP:
				...
				if (!PermissionController.hasGroup(player, group)){
					return "&4!!!&c You need to belong to group &6" + group+ "&c to use this portal";}
				break;
			case HASPERM:
