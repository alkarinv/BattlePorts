...
public class PortController {
	...
	public Port getPort(final Location l) {
		...
		if (applicablePorts == null)
			return null;
		/// If there are overlapping ports, technically we should return all of them
	...
	public void addPort(Port port){
		...
		/// Get all ports
		if (tree != null){
			tree.find(-Double.MAX_VALUE, -Double.MAX_VALUE, Double.MAX_VALUE, Double.MAX_VALUE,al);}
...
//		System.out.println("adding ports " + port + "   " + tree.getHeight() +"  " + tree.getNumberOfLeaves());
	}
	...
	public Collection<Port> getNearbyPorts(World world, Location l, int radius) {
		...
		if (tree == null)
			return null;
		ArrayList<Port> al = new ArrayList<Port>();
		tree.find(l.getBlockX()-radius, l.getBlockZ()-radius, l.getBlockX()+radius, l.getBlockZ()+radius,al);
		return al;
	}
...
//				System.out.println("Checking port " + p);
				if (p instanceof PlayerPort && ((PlayerPort)p).getOwner().equals(pname))
